...
architecture structural of processador is
    ...
    -- Sinais de Estado e Hardware
    signal estado  : std_logic;
    signal reg_ir  : unsigned(16 downto 0);
    signal borrow  : std_logic;

    -- FLAGS REGISTRADAS desv condicionais
...
begin
    ...
    -- PROGRAM COUNTER E LÓGICA DE SALTO RELATIVo
    eh_jump   <= '1' when opcode = "1111" else '0'; -- JMP Absoluto
    eh_branch <= '1' when opcode = "1110" else '0'; -- JC Relativo (Jump if Carry)

    -- A condição é atendida se a flag C (Borrow) registrada estiver em 1
    condicao_atendida <= flag_c_reg when opcode = "1110" else '0';

    -- MUX DO PC (Sequencial vs Salto Absoluto vs Salto Relativo em Complemento de 2)
    prox_pc <= 
    ...
    -- MÁQUINA DE ESTADOS E FLIP-FLOPS
    process(clk, rst)
    begin
        if rst = '1' then
            estado <= '1';
            ...
            -- Salva as flags apenas em operações aritmeticas/comparação no Execute
            if we_flags = '1' then
    ...
    prox_borrow <= flag_carry when (estado = '1' and (opcode = "0010" or opcode = "0011")) else borrow;
    ...
    offset_salto   <= reg_ir( 6 downto  0); -- Usado como Offset (Branch) ou Endereço Absoluto (Jump)
    
    imediato       <= "00000000" & reg_ir(8 downto 1); 

    banco_wr_en <= '1' when (estado = '1' and (opcode = "0001" or opcode = "0010" or opcode = "0011" or opcode = "0100" or opcode = "0101")) else '0';
    
    -- Apenas operacoes aritmeticas e CMPR (0110) alteram as flags
    we_flags    <= '1' when (estado = '1' and (opcode = "0001" or opcode = "0010" or opcode = "0011" or opcode = "0110")) else '0';

    ula_entrada_a <= x"0000" when opcode = "0101" else dado_lido_r1;
    ula_entrada_b <= imediato when opcode = "0101" else x"0000" when opcode = "0100" else dado_lido_r2;

    ula_op <=
        "001" when opcode = "0010" else   -- SUB
        "010" when opcode = "0011" else   -- SUBB
        "101" when opcode = "0100" else   -- MOV (Passa A)
        "001" when opcode = "0110" else   -- CMPR (Faz SUB e gera flags, sem escrever banco)
        "000";                            -- ADD, LI, JMP, JC

    banco : banco_regs port map(
        clk => clk, rst => rst, wr_en => banco_wr_en,
        reg_r1 => reg_fonte1, reg_r2 => reg_fonte2, reg_wr => reg_dest, data_wr => ula_resultado,
    ...
    alu : ula port map(
        entrada_A => ula_entrada_a, entrada_B => ula_entrada_b, selec_op => ula_op, borrow_in => borrow,
